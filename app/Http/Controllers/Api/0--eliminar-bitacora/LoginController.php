...
class LoginController extends Controller
{
    ...
    public function loginBitacora(Request $request)
    {
		...
		$r_usuario_tec = RolUsuario::where("id_usuario", $usuario->id_usuario)->where("id_rol_proceso", 10)->first();
		if(isset($r_usuario_tec->id_usuario)){
			$roles["tecnico"] = true;
			// return response()->json(array("respuesta"=>"error","descripcion"=>"El usuario no es técnico."));
		}
		$r_usuario_ex = RolUsuario::where("id_usuario", $usuario->id_usuario)->where("id_rol_proceso", 8)->first();
		if(isset($r_usuario_ex->id_usuario)){
			$roles["examinador"] = true;
			// return response()->json(array("respuesta"=>"error","descripcion"=>"El usuario no es técnico."));
		}
		$r_usuario_super = RolUsuario::where("id_usuario", $usuario->id_usuario)->where("id_rol_proceso", 9)->first();
		if(isset($r_usuario_super->id_usuario)){
			$roles["supervisor"] = true;
			// return response()->json(array("respuesta"=>"error","descripcion"=>"El usuario no es técnico."));
		}
		if(sizeof($roles) == 0){
			return response()->json(array("respuesta"=>"error","descripcion"=>"El usuario no tiene roles disponibles."));
		}
		...
		$arr_final["rol_user"] = $pers->run;
		$arr_final["roles"] = $roles;
