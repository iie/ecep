...
function creaListaItems(data) {
    ...
    var tablaD = $("#table-banco-items").DataTable({
        ...
        buttons: [
            {
                extend: 'excel',
                title: 'Ítems en revisión (Supervisor)',
                exportOptions: {
                    modifier: {
                        page: 'current'
                    },
        ...
        columns:[
            ...
            {data: "opciones",
                render: function(data, type, row){
                    if(row.opciones == 'Ver'){
                        //return 'Sin Asignar';
						return '<button type="button" id="item_'+row.id_item+'" class="btn _btn-item"><i class="fas fa-search"></i>&nbsp;Ver</button'
                    }
					else{
                        //return 'Asignado';
						return '<button type="button" id="item_'+row.id_item+'" class="btn _btn-item"><i class="fas fa-search"></i>&nbsp;Revisar</button'
                    }
                }
            } 
        ],
        createdRow: function( row, data, dataIndex ) {
            console.log(data)
            if ( data['corregido'] == true ) {
                $(row).addClass( 'item_corregido' );
            }
        },
        "initComplete": function(settings, json) {
            ...
            this.api().columns([2,3,4,6]).every( function (index) {
                 console.log(this)
                var column = this;
                var select = $('<select class="form-control col-sm-2 btn-light  _filtros _filtro'+index+' small"  id="select'+index+'" ><option value="" selected="selected" small>'+placeholder[index]+'</option></select>')
...
function selectColaboradores(data){
    ...
    $('#id_usuario').append('<option selected>Seleccione Colaborador</option>');
...
function redireccionarItem() {
    ...
    function redirect()    {
        location.href = serverRedirect + '/_supervisor/revisar-item.php';
    }
}
...
/*function subirArchivo() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (typeof respuesta.ok !== 'undefined') {
                showFeedback('success', respuesta.ok , 'OK');
                bancoItems();
                
            }else if (typeof respuesta.error !== 'undefined') {
                showFeedback('error', respuesta.error , 'Error');
