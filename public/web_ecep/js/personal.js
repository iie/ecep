$(document).ready(function(){
    ...
    $('#btn-jefeSede').click(verSedes);
    $('#guardar_jefeSede').click(crearJefeSede)    
    ...
    var tablaD = $("#table-postulacion").DataTable({
            ...
            select = '<select class="form-control selectEstado" name="selectEstado">'
                    if(data.deserta == false){  
                        ...
                        if(data.estado == 'capacitado'){
                            select +=   '<option value="capacitado">Capacitado</option>'
        ...
        "initComplete": function(settings, json) {
            ...
            this.api().columns([1,2,3,9,10]).every( function (index) {
                ...
                if(JSON.parse(localStorage.user).id_cargo == 1004){
                    ...
                    if(index == 9){
                        estadoUnico = arrayEstado.filter(unique)
        ...
        "drawCallback": function(settings){
            ...
            this.api().columns([1,2,3,9,10]).every( function (index) {
                ...
                }else if(JSON.parse(localStorage.user).id_cargo == 1004 && index == 9 ){
                    estadoUnico = arrayEstadoD.filter(unique)
                    estadoUnico.forEach(function(item){                      
                        $('#select'+index).append( '<option value="'+item+'">'+item+'</option>' )     
                    });
...
function llenarVista5(data){
     
    $('#filtros-jefeSede').empty();
    if($.fn.dataTable.isDataTable('#table-jefeSede')){
        $('#table-jefeSede').DataTable().destroy();
        $('#lista-jefeSede').empty();
    }
 
    var tablaD = $("#table-jefeSede").DataTable({
        dom: "<'search'f>",
        buttons: [
            {
                extend: 'excel',
        ...
        "initComplete": function(settings, json) {
            ...
            this.api().columns([1,2,3]).every( function (index) {
                var column = this;
                var select = $('<select class="form-control col-sm-2 small _filtros"  id="selectJ_'+index+'" >'+
                    '<option value="" selected="selected">'+placeholder[index]+'</option></select>')
                    .appendTo( $('#filtros-centro'))
                    .on( 'change', function () {
                        var val = $.fn.dataTable.util.escapeRegex(
                ...
                column.data().unique().each( function ( d, j ) {
                    if(d != null){
                        $('#selectJ_'+index).append( '<option value="'+d.charAt(0).toUpperCase() + d.slice(1)+'">'+d.charAt(0).toUpperCase() + d.slice(1)+'</option>' )     
                    }
                    
                } );
                 $('#selectJ_'+index).niceSelect();        
            })   
        ...
        "drawCallback": function(){
            ...
            this.api().columns([1,2,3]).every( function (index) {
                var columnFiltered = this;
                var selectFiltered = $("#selectJ_"+index)
                if(selectFiltered.val()===''){
                    selectFiltered.empty()
                    selectFiltered.append('<option value="">'+placeholder[index]+'</option>')
                    columnFiltered.column(index,{search:'applied'}).data().unique().each( function ( d, j ) {
    ...
    $('#limpiar-filtros-jefeSede').click(btnClearFilters);
    $('#total_jefeSede').html(data.length);
    $("#table-jefeSede").show();  
}
...
function llenarSelects(data){
    ...
    $('#inputRegionPostulacionJefeSede').html('').append('<option value="-1" selected="">Elegir...</option>') 
    for(i = 0; i < data.regiones_postulante.length; i++){
        $('#inputRegionPostulacionJefeSede').append('<option value="'+data.regiones_postulante[i].id_region+'">'+data.regiones_postulante[i].nombre+'</option>') 
    }
...
function agregarJefeSede(){
    limpiar()
    disabledData()

    $('#inputRegionPostulacionJefeSede').val(-1)
    $('#inputComunaPostulacionJefeSede').val(-1)
    $('#inputNivelEstudiosJefeSede').val(-1)

    $('#inputRunJefeSede').prop('disabled',false)
    $('#inputNivelEstudiosJefeSede').prop('disabled',true)
    $('#infoJefeSedeModal').modal({backdrop: 'static', keyboard: false},'show')
}

function searchRUNJefeSede() {
     
    $("#inputRunJefeSede").val($("#inputRunJefeSede").val().trim())
    if($("#inputRunJefeSede").val() == ""){
        showFeedback('error', 'Debe ingresar un RUN para realizar solicitud.', 'Error');
        return;
    }

    if($.validateRut($("#inputRunJefeSede").val()) == false) {
        showFeedback('error', 'Debe ingresar un RUN válido.', 'Error');
        return;
    }

    var run = ($('#inputRunJefeSede').val().toUpperCase()).replace(/\./g,'');
    limpiar()
    $('#inputRunJefeSede').val($.formatRut(run))
    $.blockUI({
        message: '<h1>Espere por favor</h1>',
        baseZ: 2000
    });
    ...
    $.ajax({
        method: 'POST',
        url: webservice+'/personal/obtenerJefeSede',
        headers: {
            't': JSON.parse(localStorage.user).token
        },
        crossDomain: true,
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            if(typeof mensaje["resultado"] === 'undefined'){
                console.log("correcto!")
                cargarDatosJefeSede(JSON.parse(data))
                $('#guardar_jefeSede').prop('disabled',false)
                $.unblockUI();
            }else{
                $('#guardar_jefeSede').prop('disabled',true)
                if(mensaje["resultado"] == 'existe'){
                    console.log("correcto!")
                    showFeedback("warning", mensaje["descripcion"], "");
                    disabledData()
...
function crearJefeSede(){
    Swal.fire({
          title: '¿Está seguro que desea asignarlo como Jefe de Sede?',
          type: 'warning',
          reverseButtons: true,
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          ...
          if (result.value) {
            guardarJefeSede($(this).data('id_persona'))
          }
        })

    function guardarJefeSede(persona){
        $.ajax({
            method:'POST',
            url: webservice+'/personal/guardarJefeSede',
            headers:{
               't': JSON.parse(localStorage.user).token
            },
            crossDomain: true,
...
function cargarDatosJefeSede(data){
 
    limpiar()
    disabledData()

    $('#inputRunJefeSede').val($.formatRut(data.run))
    $('#inputNombresJefeSede').val(data.nombres)
    $('#inputApellidoPaternoJefeSede').val(data.apellido_paterno)
    $('#inputApellidoMaternoJefeSede').val(data.apellido_materno)
 
    $('#inputRegionPostulacionJefeSede').val(data.id_region_postulacion == null ? -1 : data.id_region_postulacion)
    cargarComunas('inputComunaPostulacionJefeSede', data.id_region_postulacion)
    $('#inputComunaPostulacionJefeSede').val(data.id_comuna_postulacion == null ? -1 : data.id_comuna_postulacion)
 
 
    $('#inputMailJefeSede').val(data.email)
    $('#inputTelefonoJefeSede').val(data.telefono)
    $('#inputNivelEstudiosJefeSede').val(data.nivel_estudios == null ? -1 : data.nivel_estudios)
    $('#inputProfesionJefeSede').val(data.profesion)

    $('#inputEstadoJefeSede').val((data.estado == true ? 'Aprobado' : 'Rechazado'))

    $('#inputPruebaTecnica').val((data.pruebas.length != 2 ? '' : data.pruebas[1].puntaje))
    $('#inputPruebaPsicologica').val((data.pruebas.length != 2 ? '' : (data.pruebas[0].puntaje == 1 ? 'Recomendado' : data.pruebas[0].puntaje == 0 ? 'No Recomendado' : '')))
 
    $('#guardar_jefeSede').data('id_persona',data.id_persona);
...
function mostrarSedes(data){
    $('#filtros-modalJefeSede').empty();
    if($.fn.dataTable.isDataTable('#table-sedes')){
        $('#table-sedes').DataTable().destroy();
        $('#lista-sedes').empty();
    }
    jefe_sede = data.jefe_sede;
    var tablaD = $("#table-sedes").DataTable({
        dom: "<'search'f>",
             /*lengthMenu: [[10, 15, 20, -1], [10, 15, 20, "Todos"]],*/
        language:spanishTranslation,
        lengthChange: true,
        ...
        "initComplete": function(settings, json) {
            ...
            this.api().columns([0,1,2]).every( function (index) {
                var column = this;
                var select = $('<select class="form-control col-sm-2 small _filtros"  id="selectModalS'+index+'" >'+
                    '<option value="" selected="selected">'+placeholder[index]+'</option></select>')
                    .appendTo( $('#filtros-modalJefeSede'))
                    .on( 'change', function () {
                        var val = $.fn.dataTable.util.escapeRegex(
                            $(this).val()
                        );
                ...
                column.data().unique().each( function ( d, j ) {
                    if(d != null){
                        $('#selectModalS'+index).append( '<option value="'+d.charAt(0).toUpperCase() + d.slice(1)+'">'+d.charAt(0).toUpperCase() + d.slice(1)+'</option>' )     
                    }
                    
                } );
                $('#selectModalS'+index).niceSelect();    
            })   
        ...
        "drawCallback": function(){
            ...
            this.api().columns([0,1,2]).every( function (index) {
                var columnFiltered = this;
                var selectFiltered = $("#selectModalS"+index)
                if(selectFiltered.val()===''){
                    selectFiltered.empty()
                    selectFiltered.append('<option value="">'+placeholder[index]+'</option>')
                    columnFiltered.column(index,{search:'applied'}).data().unique().each( function ( d, j ) {
    ...
    $('#limpiar-filtros-modalJefeSede').click(btnClearFiltersModal);
    $('#jefeSedeModal').modal({backdrop: 'static', keyboard: false},'show') 
}
...
function cambiarEncargadoCentro(){
    ...
    $.ajax({
        method:'POST',
        url: webservice+'/centro/modificarJefeSede',
        headers:{
           't': JSON.parse(localStorage.user).token
        },
        crossDomain: true,
        ...
        data :{ 
                id_usuario: JSON.parse(localStorage.user).id_usuario,
                id_estimacion: $(this).data('id_estimacion'),
                //id_zona_region: $(this).data('id_zona_region'),
                id_jefe_sede: $(this).val() == -1 ? null : $(this).val()
            },
        success: function(data, textStatus, jqXHR) {
            data = JSON.parse(data) 
            console.log(data)
...
function btnClearFiltersModal(){
    ...
    $('#selectModalS0').val("").niceSelect('update');
    $('#selectModalS1').val("").niceSelect('update');
    $('#selectModalS2').val("").niceSelect('update'); 
    ...
    var table = $('#table-sedes').DataTable();
        table
         .search( '' )
         .columns().search( '' )
         .draw();
